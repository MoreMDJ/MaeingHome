...
import com.maeinghome.mybatisstudy.maeingpool.unpooled.UnpooledDataSource;
...
public class PooledDataSource implements DataSource {
    ...
    public void forceCloseAll() {
        synchronized (state) {
            ...
            for (int i = state.getIdleConnections().size(); i > 0; i--) {
                try {
                    PooledConnection conn = state.getIdleConnections().remove(i - 1);
                    conn.invalidate();
    ...
    protected void pushConnection(PooledConnection conn) throws SQLException {
        ...
        synchronized (state) {
            ...
            if (conn.isValid()) {
                if (state.getIdleConnections().size() < poolMaximumIdleConnections && conn.getConnectionTypeCode() == expectedConnectionTypeCode) {
            ...
            synchronized (state) {
                if (!state.getIdleConnections().isEmpty()) {
                    ...
                    if (state.getActiveConnections().size() < poolMaximumActiveConnections) {
                        ...
                        dataSource.setUrl(UserContextHolder.getDatabaseUrl());
